...
-   RStudioCloud \> New Project \> New Project from Git Repository. In "Repository URL", paste the URL of your new GitHub repository. It will be something like this: `https://github.com/j-p-courneya/GSF_GitHub_Practice.git.`
...
You can deal with git and github via the command line or a GUI (were using RStudio). The routine use of git involves just a few commands: principally add, commit, and push, but also status and diff. We will use these commands integrated into RStudio as a set of buttons and check boxes.  
...
For example, for this quarto book, I wouldn’t include all the .log, .dvi, .aux, etc., files. And if I have R code to generate a figure, I’ll include the R code but not the figure.
